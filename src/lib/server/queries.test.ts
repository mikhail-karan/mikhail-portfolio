/**
 * Acceptance criteria 9–11 from docs/analytics-spec.md §11, plus the salt upsert of §5.1 and
 * the retention rules of §7.
 *
...
describe.skipIf(!url)('against a seeded database', () => {
	...
	describe('public tier', () => {
		...
		it('returns no raw referrer and no user agent anywhere', async () => {
	...
	describe('salt acquisition', () => {
		it('hands a losing caller the salt that won, not an empty result', async () => {
			const store = createStore(() => database);
			const winner = new Uint8Array(32).fill(1);
			const loser = new Uint8Array(32).fill(2);

			// What `on conflict do nothing` would break: the second caller returns no row and
			// the region it is serving has no salt at all (§5.1).
			expect(await store.claimSalt('2026-08-06', winner)).toEqual(winner);
			expect(await store.claimSalt('2026-08-06', loser)).toEqual(winner);
		});

		it('destroys earlier days as soon as the day rolls over', async () => {
			const store = createStore(() => database);

			await store.claimSalt('2026-08-05', new Uint8Array(32).fill(1));
			await store.claimSalt('2026-08-06', new Uint8Array(32).fill(2));

			const rows = await database.select().from(salt);
			expect(rows.map((row) => row.day)).toEqual(['2026-08-06']);
		});
	});
	...
	describe('retention sweep', () => {
		it('nulls old user agents without deleting the rows', async () => {
			...
			await database.insert(event).values([
				{ ...row, ts: new Date(Date.now() - (UA_RETENTION_DAYS + 1) * DAY_MS) },
				{ ...row, ts: new Date(), visitorDay: 'fresh-visitor' },
			]);

			expect((await sweep(database)).scrubbed).toBe(1);
		...
		it('destroys salts for past days', async () => {
			...
			expect((await sweep(database)).saltsDropped).toBe(1);
